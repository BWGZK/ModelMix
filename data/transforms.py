...
def pad(image, target, region):
    z,x,y = image.shape
    scaler = np.random.uniform(0.6, 1.4)
    nx,ny = region
    nx = int(nx*scaler)
    ny = int(ny*scaler)
    if abs(x-nx) >0:
...
class RandomRotate(object):
    ...
    def __call__(self, img, target):
        ...
        img = img.copy()
        rotated_img = F.rotate(torch.from_numpy(img), angle1, PIL.Image.NEAREST, self.expand, self.center)
        # rotated_img_aux = F.rotate(torch.from_numpy(img), angle2, PIL.Image.NEAREST, self.expand, self.center)
        # rotated_img_aux = rotated_img_aux.numpy()
        rotated_img = rotated_img.numpy()
        mask = target['masks']
        mask = mask.copy()
        # rotated_mask = F.rotate(torch.from_numpy(mask), angle1, PIL.Image.NEAREST, self.expand, self.center)
        # rotated_mask_aux = F.rotate(torch.from_numpy(mask), angle2, PIL.Image.NEAREST, self.expand, self.center)
        # rotated_mask_aux = rotated_mask_aux.numpy()
        # rotated_mask = rotated_mask.numpy()
        
        # beta = np.random.beta(0.5, 0.5, [1,1,1])
        # mix_img = (1-beta)*rotated_img_aux+beta*rotated_img
        ...
        mask[:,x_min_cut:x_max_cut, y_min_cut:y_max_cut] = 0
...
class RandomRotate(object):
    ...
    def __call__(self, img, target):
        ...
        # target_size = random.randint(min(100, img.shape[1]), max(100,img.shape[1]))
        # target_size = random.randint(0, img.shape[1])
        ...
        y_max_cut = min(y_min_cut+32, img.shape[1])
        rotated_img[:,x_min_cut:x_max_cut, y_min_cut:y_max_cut] = 0
        rotated_mask[:,x_min_cut:x_max_cut, y_min_cut:y_max_cut] = 0
